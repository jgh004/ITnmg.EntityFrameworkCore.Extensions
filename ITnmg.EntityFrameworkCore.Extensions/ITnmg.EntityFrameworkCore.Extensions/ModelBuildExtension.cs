            ...
            var configs = assembly.GetTypes()
                .Where( f =>
                {
                    return f.IsClass && !f.IsAbstract && !f.IsGenericType
                        && f.GetInterfaces().Where( i => i.IsInterface && i.GetGenericTypeDefinition() == typeof( IEntityTypeConfiguration<> ) ).Count() > 0;
                } )
                .Select( t => new
                {
                    //IEntityTypeConfiguration 接口的泛型参数，即实体类型
                    EntityType = t.GetInterfaces().First( i => i.GetGenericTypeDefinition() == typeof( IEntityTypeConfiguration<> ) ).GenericTypeArguments[0],
